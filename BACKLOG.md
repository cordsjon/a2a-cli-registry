...
- [ ] AC-01: `registry-inspect` prints schema/tag-coverage/type-coverage/path-resolution stats for an explicitly-named DB path (help text names the two DBs — a2a-cli-registry/registry.db vs ~/.hermes/cli-registry.db — to prevent wrong-DB recon)
  ...
  > Note (2026-09-09, commit 5343df8): **there is no longer a second DB to name.** Both in-repo copies are deleted — the untracked `registry.db` (479 rows) and the tracked `registry.db.bak-40866b1` (474 rows, swept in by a nightly janitor auto-commit because `*.db` does not match a suffix appended after the extension). `.gitignore` now anchors on `registry.db*`. Neither was merely stale: both lack the `playbook_sig`/`playbook_fts*` tables the live registry has, so they predate the playbook feature. AC-01's "help text names the two DBs" wording is now obsolete — drop that clause and scope the AC to coverage stats, as the note above already recommends.
