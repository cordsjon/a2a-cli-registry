...
- **`confidence: "declared"` is mandatory on BOTH rows, not optional.** The planner's `_hop_excluded` (`core/planner/search.py:71-74`) excludes any `writes-fs`/`network` hop that is carried by an **inferred** capability, even when the caller has not restricted side-effects. The registry default confidence is inferred (rank ≥ 1). If the feed omits `confidence`, the "safe" `split`/`compress`/`convert` verbs are silently pruned from every unguarded plan — the exact opposite of the intended behaviour. Declaring the capability marks the blast radius as operator-verified so `writes-fs` flows freely; only `destructive` then requires `allow_side_effects`.
