...
- **Manipulation:** only `merge` exists (SVG-PAINT `collection_overview_pdf_service.py`, pypdf) plus two narrow OCR tools (`mistral_ocr` = cloud API; `termux-ocr-server` = image-only). **Five of eight manipulation verbs have zero tooling anywhere in the fleet:** `split`, `redact`, `compress`, `form-fill`, and local `convert`. No `pikepdf`/`ocrmypdf`/`ghostscript`/`qpdf` exists in any project.
...
- **Safe vs. destructive split is mandatory:** the planner *excludes* `destructive` verbs unless the caller passes `allow_side_effects`. `redact`/`form-fill`/`encrypt` mutate/remove content → `destructive`. `split`/`compress`/`convert` → `writes-fs`.
...
- **AC-4 (side-effect gate):** `redact`/`form-fill` are **excluded** from an unguarded plan and **included** only with `allow_side_effects`. Verified via two `plan_cli_chain` calls.
