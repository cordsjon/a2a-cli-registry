...
def _pick(declared_val, inferred_val):
    ...
    NOTE: relies on Python truthiness; safe for list[str] and str. Do NOT add
...
def merge_capabilities(declared, inferred):
    ...
    Result confidence is 'declared' ONLY when a declared field actually
    survived into the merge — otherwise it is the inferred confidence.
    (A fully-inferred merge mislabeled 'declared' would bypass the §8
    fail-UNSAFE guard, which gates on confidence.)"""
    if declared is None:
        return inferred
    if inferred is None:
        return declared
    ...
    # _pick returns one of its two argument objects unchanged, so identity
    # comparison reliably detects which source won each field.
