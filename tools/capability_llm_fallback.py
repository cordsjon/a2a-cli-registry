...
_SYSTEM = (
    ...
    "side_effect rules: 'writes-fs' if the tool writes to a database, "
    "modifies files in place, or writes files beyond an explicitly "
    "requested output path (seeders, migrators, and DB inserts are always "
    "'writes-fs'); 'network' if it calls remote APIs or serves requests; "
    "'destructive' if it deletes or drops data; 'none' for read-only "
    "tools and pure converters that only write the output file the caller "
    "asked for. output_types: use 'path' for any file the tool writes, "
    "including binary formats like PDF or PPTX. "
